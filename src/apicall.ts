...
import { DivisionResposne, Division, MemberVoting, Vote } from './models/divisions';
...
/**
 * @param startDate - required format yyyy-MM-dd 
 */
export const getCommonsVotesAfterDate = async (startDate: string): Promise<Array<Division>> => {

  const url: string = `https://commonsvotes-api.parliament.uk/data/divisions.json/search?queryParameters.startDate=${startDate}`;  
  logger.trace(url);
  ...
  try {    
    ...
    logger.error(`Failed to get votes for url ${url}`, error);    
    throw error;
  }
